...
BST* BSTRead(string filename,BST* root)
{
	ifstream A(filename, ios::binary);
	if (!A.is_open())
	{
	...
	{
		i++;
		root=BSTinsert(root, a.car_number, i);
	}
	if (A.bad())
	{
		cout << "Ошибка аутпута\n";
...
ST* STRead(string filename, ST* root)
{
	ifstream A(filename, ios::binary);
	if (!A.is_open())
	{
	...
	A.read((char*)&a, sizeof(PDD));
	delete root;
	root = new ST(a.car_number, i);
	ST::insertCount++;
	while (A.read((char*)&a, sizeof(PDD)))
	{
		i++;
		ST::insertCount++;
		root=STinsert(root, a.car_number, i);
	}
	if (A.bad())
	{
		cout << "Ошибка аутпута\n";
...
void BSTprint2D(BST* root, int delim)
{
	...
	BSTprint2D(root->right, delim);

	cout << endl << endl << space * (delim + 5) << root->key << "\n\n";
	BSTprint2D(root->left, delim);
}
...
void STprint2D(ST* root, int delim)
{
	...
	STprint2D(root->right, delim);

	cout << endl << endl << space * (delim + 5) << root->key << "\n\n";
	STprint2D(root->left, delim);
}